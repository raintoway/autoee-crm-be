...
public interface CrmBusinessService {
	...
	default String getBusinessStatusName(Integer endStatus, CrmBusinessStatusDO status) {
		if (endStatus != null) {
			// 使用 Optional 避免空指针
			return Optional.ofNullable(CrmBusinessEndStatusEnum.fromStatus(endStatus))
					.map(CrmBusinessEndStatusEnum::getName)
					.orElse("未知状态"); // 或返回空字符串/默认值
		}
		return status != null ? status.getName() : "无状态信息";
	}
